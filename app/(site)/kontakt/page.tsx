      ...
      <section className="bg-white pt-10 pb-16 lg:pb-24">
        <Container>
          <nav className="flex items-center gap-1.5 text-xs text-[var(--text-tertiary)] mb-8">
            ...
            <span>/</span>
            <span className="text-[var(--text-secondary)]">Kontakt</span>
          </nav>

          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 lg:gap-20 items-start">

            <div>
              <p data-reveal className="text-xs font-semibold uppercase tracking-[0.2em] text-[#00a5ec] mb-4">Kontakt</p>
              <SplitText as="h1" trigger="mount" className="text-3xl sm:text-4xl font-bold tracking-[-0.025em] text-[var(--text-primary)] leading-tight mb-2">
                Offerte anfragen.
              </SplitText>
              <p data-reveal className="text-base text-[var(--text-secondary)] leading-relaxed mb-8 max-w-lg">
            ...
            <div data-reveal className="lg:sticky lg:top-24 space-y-6">
              ...
              <div className="bg-[var(--bg-secondary)] rounded-[16px] p-6">
                <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[var(--text-tertiary)] mb-4">Ihr Ansprechpartner</p>
                <div className="flex items-start gap-4">
                  <div className="shrink-0 w-16 h-[74px] rounded-[10px] overflow-hidden bg-[var(--bg-tertiary)]">
                    <Image
                      src={portraitUrl}
                  ...
                  <div>
                    <p className="text-sm font-bold text-[var(--text-primary)]">{person?.name ?? 'Peter Keller'}</p>
                    <p className="text-xs text-[var(--text-tertiary)] mt-0.5 mb-3">{person?.titel ?? 'Geschäftsführer'}</p>
                    <a href={`tel:${telefon.replace(/\s/g, '')}`} className="text-sm font-semibold text-[#0091d4] hover:text-[#007ab8] transition-colors">
                      {telefon}
                    </a>
                  </div>
                </div>
              ...
              <div className="bg-[var(--bg-secondary)] rounded-[16px] p-6 space-y-4">
                <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[var(--text-tertiary)]">Adresse</p>
                <div className="text-sm text-[var(--text-secondary)] leading-relaxed">
                  <p className="font-semibold text-[var(--text-primary)]">{firmenname}</p>
                  <p>{strasse}</p>
                  <p>{plzOrt}</p>
                  <p>Schweiz</p>
                </div>
                <div className="pt-2 space-y-2 text-sm">
                  <a href={`tel:${telefon.replace(/\s/g, '')}`} className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
                  ...
                  <a href={`mailto:${email}`} className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="shrink-0 text-[var(--text-tertiary)]">
              ...
              <div className="bg-[var(--bg-secondary)] rounded-[16px] p-6">
                <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[var(--text-tertiary)] mb-4">Öffnungszeiten</p>
                <div className="space-y-2 text-sm text-[var(--text-secondary)]">
                  {oeffnungszeiten.map(({ tag, zeit }) => (
                    <div key={tag} className="flex justify-between gap-4">
                      <span className="font-medium text-[var(--text-primary)] shrink-0">{tag}</span>
