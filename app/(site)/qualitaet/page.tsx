      ...
      <section className="bg-white pt-10 pb-0">
        <Container>
          <nav className="flex items-center gap-1.5 text-xs text-[var(--text-tertiary)] mb-8">
            ...
            <span>/</span>
            <span className="text-[var(--text-secondary)]">Qualität</span>
          </nav>
        ...
        <Container>
          <div data-reveal-stagger className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map(({ value, label }) => (
              <div key={label} className="bg-white/[0.06] rounded-[12px] px-6 py-8 flex flex-col items-center text-center">
          ...
          <div data-reveal-stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {schritte.map(({ nr, title, body }) => (
              <div key={nr} className="bg-white rounded-[14px] p-6">
                <span className="text-2xl font-black text-[var(--bg-tertiary)] tabular-nums leading-none">{nr}</span>
                <h3 className="text-base font-semibold text-[var(--text-primary)] mt-3 mb-2 leading-snug">{title}</h3>
        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-10 lg:gap-20">
            <div>
              ...
              <p data-reveal className="mt-4 text-sm text-[var(--text-secondary)] leading-relaxed max-w-xs">
            ...
            <div data-reveal-stagger className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {normenItems.map(({ code, beschreibung }) => (
                <div key={code} className="bg-[var(--bg-secondary)] rounded-[12px] px-5 py-4">
                  <p className="text-sm font-bold text-[var(--text-primary)]">{code}</p>
                  <p className="text-xs text-[var(--text-tertiary)] mt-0.5">{beschreibung}</p>
                </div>
              ))}
            </div>
          </div>
