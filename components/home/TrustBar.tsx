...
const visuals = [
  (
    <div className="absolute inset-0 flex items-center justify-center p-8">
      <div className="bg-white rounded-lg w-full max-w-[220px] overflow-hidden shadow-[0_2px_12px_rgba(15,17,23,0.08)]">
        <div className="h-2.5 bg-brand-500 flex items-center px-3">
        ...
        <div className="h-20 bg-[#ecedef] flex flex-col justify-center px-3">
          <span className="text-[10px] text-(--text-secondary) font-medium">Grundwerkstoff · Stahl</span>
        </div>
        <div className="px-3 py-2.5 flex items-center justify-between">
          <span className="text-[9px] font-mono text-(--text-tertiary)">5–50 µm</span>
          <span className="text-[9px] text-brand-500 font-semibold">✓ Toleranz eingehalten</span>
        </div>
      </div>
    </div>
  ),
  (
    <div className="absolute inset-0 flex items-center justify-center p-8">
      <div className="bg-white rounded-lg w-full max-w-[220px] p-4 shadow-[0_2px_12px_rgba(15,17,23,0.08)]">
        <p className="text-[10px] font-semibold text-(--text-tertiary) uppercase tracking-widest mb-3">Beratungsprozess</p>
        {[
          ...
          <div key={label} className="flex items-center gap-2.5 py-1.5">
            <div className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${done ? 'bg-brand-500' : 'bg-(--bg-secondary)'}`}>
              {done && (
            ...
            <span className={`text-xs ${done ? 'text-(--text-secondary)' : 'text-(--text-tertiary)'}`}>{label}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="bg-white rounded-lg w-full max-w-[220px] p-4 shadow-[0_2px_12px_rgba(15,17,23,0.08)]">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={sub} className="bg-(--bg-secondary) rounded-[6px] px-2.5 py-2">
    ...
    <section className="bg-white">
      <Container>
        <div data-reveal-stagger className="grid grid-cols-1 md:grid-cols-3 gap-4 py-14">
          {merged.map((item, i) => (
            <div key={i}>
              <div className="bg-(--bg-secondary) rounded-[16px] h-[220px] relative overflow-hidden">
                {visuals[i]}
              </div>
              <div className="mt-5 px-1">
                <h3 className="text-base font-bold text-(--text-primary) leading-snug tracking-[-0.01em]">
